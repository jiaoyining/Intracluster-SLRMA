...
DATA{M+2}=load(['./',METHODNAME{M+2},'/pcolor/',METHODNAME{M+2},'_',DATABASE,'_',NOISE,'_',num2str(NI),'_',num2str(PSNR(NB)),'.mat']);
DATA{M+2}=DATA{M+2}.IM;
end
...
[R,C]=size(PIC_INIT);
if NI==1
POSITION1=[30,100,20,20];
POSITION2=[125,80,20,20];
else
POSITION1=[15,80,20,20];
POSITION2=[120,120,20,20];
end

POSITION3=[1,R,C/2,C/2];
POSITION4=[C/2,R,C/2,C/2];

%FILENAME=[new_folder,'\',METHODNAME{M},'_',DATABASE,'_',num2str(NI),'_','_BAND',num2str(BAND),'.pdf'];
%export_fig (FILENAME);
...
REGION1= imresize(CHOSEN1,[R/2 C/2]);
%imshow(REGION1); 
%FILENAME1=[new_folder,'\',METHODNAME{M},'_',DATABASE,'_',num2str(NI),'_','_BAND',num2str(BAND),'_SUB1','.pdf'];
%export_fig (FILENAME1);
%[R,C]=size(REGION1);
%REGION1=rectangle('Position',[1,1,R-1,C-1],'EdgeColor','r','LineWidth',1.5);
REGION2= imresize(CHOSEN2,[R/2 C/2]);
%% drawing boxes 
if rem(R,2)==0
PIC_COMPLETED=[PIC_INIT;REGION1,REGION2];
else
PIC_COMPLETED=[PIC_INIT;REGION1,1,REGION2];
end
figure(1);
K=imadjust(PIC_COMPLETED,[max(0,min(min(PIC_INIT(:)))),min(1,max(max(PIC_INIT(:))))],[0,1]);
%imshow(PIC_COMPLETED,[]);
I=imshow(K);
