function [Y,BregD,rCov,lam,Sigma_Y,k] = RLPHCS_Cov_Rec_structure(A, D, F, opts, oCov)
% =========================================================================
% DESCRIPTION:
...
while iterNum < opts.maxIterNum %&& RelChgOut_Y > opts.tol || iterNum < 1
    ...
    temX = Y .* sqnp;
    rCov = cov(temX');
    BregD(iterNum) = CovarianceLoss(rCov, oCov);
    
    %if opts.print
        fprintf('iterations %d, BregD %.5f...\n',iterNum,BregD(iterNum));
    %end
end
BregD = BregD(1 : iterNum);
end